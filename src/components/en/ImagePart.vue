...
<template>
  <div>
      <q-card>
        <img v-if="en.image" :src="'data:image/png;base64,'+en.image" ref="img_tag" style="display:none">
...
function sketchpad_mouseUp() {
    mouseDown=0;
    en.image = canvas_to_image(canvas)
}
...
function event_listen(canvas_, ctx_, en_) {
  canvas = canvas_
  ctx = ctx_
  en = en_
...
export default {
  ...
  mounted () {
    if (!this.en.image) {
      this.searchEnImage(this.update_canvas)
    }
  },
  computed: {
    ...mapGetters(['en'])
    ...
    upload (event) {
      ...
      reader.onload = event => {
        ...
        img.onload = () => {
          ...
          this.en.image = canvas_to_image(canvas)
          event_listen(canvas, ctx, this.en)
        }
